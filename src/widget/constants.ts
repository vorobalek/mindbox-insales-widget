export const FAVORITES_EVENT = 'update_items:insales:favorites_products';
export const CART_EVENT = 'update_items:insales:cart:light';

export const MESSAGE_WIDGET_NOT_CONFIGURED = '[mindbox] Виджет не настроен. Проверьте обязательные поля.';
export const MESSAGE_TRACKER_NOT_INITIALIZED = '[mindbox] Трекер не инициализирован. Проверьте apiDomain.';
export const MESSAGE_EVENTBUS_UNAVAILABLE = '[mindbox] EventBus недоступен, подписки не активированы.';
export const MESSAGE_MISSING_SETTINGS = '[mindbox] Не заполнены обязательные настройки виджета:';
export const AUTHORIZE_CUSTOMER_OPERATION = 'Website.AuthorizeCustomer';
export const AUTHORIZE_CUSTOMER_SESSION_KEY = 'mindbox_insales_authorize_customer_website_id';
export const AUTHORIZE_CUSTOMER_RETRY_ATTEMPTS = 15;
export const AUTHORIZE_CUSTOMER_RETRY_DELAY_MS = 400;

export const RETRY_ATTEMPTS = 50;
export const RETRY_DELAY_MS = 100;
